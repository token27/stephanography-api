module.exports = (function () {
    ...
    function _getPicamoSection(file, callback) {
        ...
        imageFile.on('readable', () => {
            ...
            while (null !== (chunk = imageFile.read(1))) {
                ...
                } else if (result.compressed == null) {
    ...
    var pack = function (options, callback) {
        var packResult = {success: false, message: null, hat: null},
            combinedStream = CombinedStream.create(),
            image = util.format('%s.enc.jpg', options.image),
            input = fs.createReadStream(options.image),
            output = fs.createWriteStream(image),
            hatFile = util.format('%s.enc.key', options.image);
        ...
        output.on('finish', function () {
            ...
            packResult.message = util.format(
                ...
                util.format(
                    ...
                    options.imageSize,
                    size,
                    Math.round((size / options.imageSize) * 100) / 100
                ),
                options.paranoia
        ...
        //build stream
        _buildHideStream(options, (buildResult) => {
